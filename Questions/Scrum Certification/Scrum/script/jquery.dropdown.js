...
(function ($) {
  ...
  var action = {
    ...
    singleChoose: function (event) {
      ...
      $.each(_config.data, function (key, item) {
        ...
        if ('' + item.id === '' + value) {
          ...
          if (item.selected) {
            _dropdown.name.push('<span class="dropdown-selected">' + item.name + '</span>');
          }
        }
      });
    ...
    del: function (event) {
      ...
      _dropdown.$el.find('[value="' + id + '"]').prop('selected', true).removeAttr('selected');
